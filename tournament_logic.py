    ...
    def create_matchups(self, num_rounds: int):
        ...
        for j in range(num_rounds):
            ...
            while len(player_match_count[j]) > 1:
                ...
                if sorted([player_match_count[j][random_idxs[0]],
                           player_match_count[j][random_idxs[1]]]) not in all_matches:
                    matchup = [player_match_count[j].pop(i) for i in sorted(random_idxs, reverse=True)]
                    ...
                    print('This combination is a duplicate of an earlier matchup: ',
                          player_match_count[j][random_idxs[0]], 'vs. ', player_match_count[j][random_idxs[1]])
                    fail_count += 1
                    if fail_count > 5:
                        break
